#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2019/05/27 10:37
# @Author  : 852782749@qq.com
# @File    : nsfocus_bug_excel_update.py
# @Software: PyCharm
...
def vnln_hosts(bug_hosts):
    ...
    bug_host_r = r'vulnhostHtml/hosts_(.*?).html'
    # soup=BeautifulSoup(open('bug_host_r'),"lxml")
    bug_host_r_re = re.compile(bug_host_r)
    #print bug_host_r_re
    bug_host_r_list = re.findall(bug_host_r_re, bug_hosts)
    #print len(bug_host_r_list)
    for num1 in range(len(bug_host_r_list)):
#    for num1 in range(12,13):
        html1 = 'vulnhostHtml/hosts_' + bug_host_r_list[num1] + '.html'
        #print html1
        soup1 = BeautifulSoup(open(html1), "lxml")
        data1 = str(soup1.find_all("table", {"class": 'report_table'})).decode('unicode_escape')
        data1 = filter(lambda x: x not in ['\n', '\r', ' ', '\t'], data1)
        #print data1
        re_host_r = r'host/(.*?).html'
        re_host_r_r = re.compile(re_host_r)
        re_host_r_list = re.findall(re_host_r_r, data1)
        #print len(re_host_r_list)
        return re_host_r_list
